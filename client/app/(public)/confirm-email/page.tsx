...
export default async function ConfirmEmailPage({ searchParams }: Props) {
    ...
    return (
      <div className="min-h-screen bg-(--bg-base) flex items-center justify-center px-4">
        <div className="max-w-sm text-center">
          ...
          <p className="text-sm text-(--text-muted)">This confirmation link is missing a token.</p>
        </div>
      </div>
    );
  }
  ...
  try {
    const res = await fetch(`${apiUrl}/public/confirm-email?token=${token}`);
      ...
      <div className="max-w-sm text-center">
        ...
        {success && (
          <p className="text-xs text-(--text-muted)">
            You will receive email updates when new changelogs are published.
          </p>
        )}
      </div>
    </div>
